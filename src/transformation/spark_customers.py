"""
spark_customers.py — Bronze → Silver transformation for customers.

Transformations:
  - Lowercase + strip whitespace from customer_city
  - Deduplicate on customer_id
  - Write to data/silver/customers.parquet
"""
...
from src.config import BRONZE_PATH, SILVER_CUSTOMERS
...
def transform_customers(
    ...
    output_path: Path = SILVER_CUSTOMERS,
) -> DataFrame:
    if spark is None:
    ...
    csv_path = str(bronze_folder / "olist_customers_dataset.csv")
    logger.info("Reading customers from: %s", csv_path)

    df = spark.read.csv(csv_path, header=True, inferSchema=True)
    logger.info("Raw customers count: %d", df.count())
    ...
    logger.info("Customers silver written to: %s", output_path)
...
if __name__ == "__main__":
    ...
    df.printSchema()
    print(f"Silver customers row count: {df.count()}")
